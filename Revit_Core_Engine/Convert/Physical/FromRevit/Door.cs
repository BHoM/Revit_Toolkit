...
namespace BH.Revit.Engine.Core
{
    public static partial class Convert
    {
        ...
        public static Door DoorFromRevit(this FamilyInstance familyInstance, HostObject host = null, RevitSettings settings = null, Dictionary<string, List<IBHoMObject>> refObjects = null)
        {
            ...
            {
                if (host == null)
                    BH.Engine.Base.Compute.RecordWarning(String.Format("Location of the door could not be retrieved from the model (possibly it has zero area or lies on a non-planar face). A door object without location has been returned. Revit ElementId: {0}", familyInstance.Id.Value()));
                else
                {
                    BH.Engine.Base.Compute.RecordWarning(String.Format("Location of the door could not be retrieved from the model (possibly it has zero area or lies on a non-planar face), the opening has been skipped. Revit ElementId: {0}", familyInstance.Id.Value()));
                    return null;
                }
            }
