    ...
    public static partial class Query
    {
        ...
        public static IEnumerable<ElementId> ElementIdsOfViewTemplates(this Document document, string viewTemplateName = null, bool caseSensitive = true, IEnumerable <ElementId> ids = null)
        {
            ...
            if (!string.IsNullOrEmpty(viewTemplateName))
            {
                ...
                if (result.Count() == 0)
                    BH.Engine.Base.Compute.RecordWarning("Couldn't find any View Template named " + viewTemplateName + ".");
                else if (result.Count() != 1)
                    BH.Engine.Base.Compute.RecordWarning("More than one View Template named " + viewTemplateName + " has been found.");
