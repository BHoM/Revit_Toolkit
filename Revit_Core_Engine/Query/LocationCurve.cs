...
namespace BH.Revit.Engine.Core
{
    public static partial class Query
    {
        ...
        public static ICurve LocationCurve(this FamilyInstance familyInstance, RevitSettings settings = null)
        {
            ...
            else if (typeof(IFramingElement).BuiltInCategories()
                .Contains((BuiltInCategory) familyInstance.Category.Id.IntegerValue))
        ...
        public static ICurve LocationCurveFraming(this FamilyInstance familyInstance, RevitSettings settings = null)
        {
            ...
            BH.oM.Geometry.Line transformedLine = BH.Engine.Geometry.Create.Line(transform.Origin.PointFromRevit(),
                transform.BasisX.VectorFromRevit());
            line = new BH.oM.Geometry.Line
            {
                Start = transformedLine.PlaneIntersection(startPlane, true),
                End = transformedLine.PlaneIntersection(endPlane, true)
            };
        ...
        public static BH.oM.Geometry.Line LocationCurveColumn(this FamilyInstance familyInstance, RevitSettings settings = null)
        {
            ...
            else
            {
                ...
                if (baseLevelParam != null)
                {
                    ...
                    double baseLevel = (familyInstance.Document.GetElement(baseLevelParam.AsElementId()) as Level)
                        .ProjectElevation;
                    double topLevel = (familyInstance.Document.GetElement(topLevelParam.AsElementId()) as Level)
                        .ProjectElevation;
            ...
            if (curve != null)
            {
                ...
                if (Math.Abs(startExtension) > settings.DistanceTolerance ||
                    Math.Abs(endExtension) > settings.DistanceTolerance)
                {
        ...
        public static List<BH.oM.Geometry.Line> LocationCurveMEP(this FamilyInstance mepInstance, RevitSettings settings = null)
        {
            //this method queries the line equivalent of mep fittings only
            //by getting its connectors points and creating a line between
            
            List<BH.oM.Geometry.Line> result = new List<BH.oM.Geometry.Line>();
            
            if (mepInstance.Category.Id != new ElementId(Autodesk.Revit.DB.BuiltInCategory.OST_DuctFitting)&&
                mepInstance.Category.Id != new ElementId(Autodesk.Revit.DB.BuiltInCategory.OST_PipeFitting)&&
                mepInstance.Category.Id != new ElementId(Autodesk.Revit.DB.BuiltInCategory.OST_CableTrayFitting))
            {
                BH.Engine.Reflection.Compute.RecordError(string.Format("Revit object is of category {0}. This method can only query family instances of categories Duct Fitting, Pipe Fitting or Cable Tray fitting", mepInstance.Category.Name));
            }
            ...
            if (mepModel != null)
            {
                ...
                {
                    result.Add(BH.Engine.Geometry.Create.Line(connectors[0].Origin.PointFromRevit(),connectors[1].Origin.PointFromRevit()));
                }
                else
                {
                    BH.oM.Geometry.Point origin = ((mepInstance.Location) as LocationPoint).Point.PointFromRevit();
                    foreach (Connector c in connectors)
                    {
                        BH.oM.Geometry.Line line = BH.Engine.Geometry.Create.Line(origin, c.Origin.PointFromRevit());
                    }    
                }
            }
        ...
        public static List<BH.oM.Geometry.Line> LocationCurveMEP(this MEPCurve mepCurve, RevitSettings settings = null)
        {
            ...
            //hence the best approach is to query the connections that are not end, to split curve there
        ...
        [Input("isStartConnected",
            "An OUT operator to also extract whether the start of the MEPCurve is connected to something.")]
        [Input("isEndConnected",
            "An OUT operator to also extract whether the end of the MEPCurve is connected to something.")]
                        ...
                        for (int j = 0; j < c2Connectors.Count; j++)
                        {
                            ...
                            if (c2.Owner.Id != c1.Owner.Id)
                            {
                                if (!(c2.Owner is MEPCurve))
                                {
                                    ...
                                    List<Connector> c3Connectors =
                                        (connectorManager1.Connectors).Cast<Connector>().ToList();
                                    if (c3Connectors.Count == 2)
                                    {
                                        for (int k = 0; k < c3Connectors.Count; k++)
                                        {
                                            ...
                                            if (c3.Origin.DistanceTo(c1.Origin) > tolerance)
                                            {
                                                if (c3.IsConnected)
                                                {
                                                    List<Connector> c4Connectors =
                                                        (c3.AllRefs).Cast<Connector>().ToList();
                                                        ...
                                                        if (c4.Owner.Id != c2.Owner.Id && c4.Owner.Id != c1.Owner.Id)
                                                        {
                                                            if (!(c4.Owner is MEPCurve))
                                                            {
                                                                FamilyInstance familyInstance2 =
                                                                    c4.Owner as FamilyInstance;
                                                                ConnectorManager connectorManager2 =
                                                                    familyInstance2.MEPModel.ConnectorManager;
                                                                List<Connector> c5Connectors =
                                                                    (connectorManager2.Connectors).Cast<Connector>()
                                                                    .ToList();
                                                                if (c5Connectors.Count == 2)
                                                                {
                                                                    for (int m = 0; m < c5Connectors.Count; m++)
                                                                    {
                                                                        ...
                                                                        {
                                                                            locationToSnap =
                                                                                BH.Engine.Geometry.Modify
                                                                                    .RoundCoordinates(
                                                                                        Convert.PointFromRevit(
                                                                                            c5.Origin), 4);
                                                                            BH.oM.Geometry.Line longLine =
                                                                                BH.Engine.Geometry.Create.Line(
                                                                                    locationToSnap, locationFromSnap);
                                                                            locationToSnap =
                                                                                BH.Engine.Geometry.Modify
                                                                                    .RoundCoordinates(
                                                                                        longLine.PointAtParameter(0.5),
                                                                                        4);
                                                                ...
                                                                else
                                                                {
                                                                    ...
                                                                    locationToSnap =
                                                                        BH.Engine.Geometry.Modify.RoundCoordinates(
                                                                            Convert.FromRevit(
                                                                                (familyInstance2.Location) as
                                                                                LocationPoint), 4);
                                                                    BH.oM.Geometry.Line shortLine =
                                                                        BH.Engine.Geometry.Create.Line(locationToSnap,
                                                                            locationFromSnap);
                                                                    endPoints.Add(
                                                                        BH.Engine.Geometry.Modify.RoundCoordinates(
                                                                            Convert.PointFromRevit(c1.Origin), 4));

                                                                    if (c1.Id == 0)
                                                                    {
                                                                        extraLine1 = shortLine;
                                                                        if (c5Connectors.Any(x =>
                                                                            x.IsConnected == true))
                                                                            isStartConnected = true;
                                                                    }
                                                                    else
                                                            ...
                                                            else
                                                            {
                                                                endPoints.Add(
                                                                    BH.Engine.Geometry.Modify.RoundCoordinates(
                                                                        Convert.FromRevit(
                                                                            (c3.Owner.Location) as LocationPoint), 4));
                                                                if (c1.Id == 0)
                                                                    isStartConnected = true;
                                                                else
                                                                    isEndConnected = true;
                                                ...
                                                else
                                                {
                                                    ...
                                                    locationToSnap =
                                                        BH.Engine.Geometry.Modify.RoundCoordinates(
                                                            Convert.PointFromRevit(c3.Origin), 4);
                                                    BH.oM.Geometry.Line shortLine =
                                                        BH.Engine.Geometry.Create.Line(locationToSnap,
                                                            locationFromSnap);
                                    ...
                                    else
                                    {
                                        ...
                                        endPoints.Add(BH.Engine.Geometry.Modify.RoundCoordinates(
                                            Convert.FromRevit((familyInstance1.Location) as LocationPoint), 4));
                                        if (c1.Id == 0)
                                        {
                                ...
                                {
                                    endPoints.Add(
                                        BH.Engine.Geometry.Modify.RoundCoordinates(Convert.PointFromRevit(c1.Origin),
                                            4));
                                    if (c1.Id == 0)
                                    {
        ...
        */
        ...
        public static List<BH.oM.Geometry.Line> LocationCurveMEP1(this MEPCurve mepCurve, out bool isStartConnected, out bool isEndConnected, RevitSettings settings = null)
        {            
            settings = settings.DefaultIfNull();
            ...
            for (int i = 0; i < c1Connectors.Count; i++)
            {
                ...
                if (c1.ConnectorType == ConnectorType.End)
                {
                    if (c1.IsConnected)
                    {
                        endPoints.Add(BH.Engine.Geometry.Modify.RoundCoordinates(Convert.PointFromRevit(EndChainPoint(c1)),4));
                    }
                    else
                    {
                        endPoints.Add(BH.Engine.Geometry.Modify.RoundCoordinates(Convert.PointFromRevit(c1.Origin),4));
        ...
        private static XYZ EndChainPoint(Connector initialConnector, RevitSettings settings = null)
        {
            settings = settings.DefaultIfNull();
            
            // get all references from current connector
            List<Connector> connectors = (initialConnector.AllRefs).Cast<Connector>().ToList();
            
            // get the connector connecting to initialconnector
            Connector nextConnector = connectors.First(x => x.Owner.Id != initialConnector.Owner.Id);
            
            // get child element
            Element child = nextConnector.Owner;

            // if next element is another mep curve, then it was connected without fittings and we can just return conn location
            if (child is MEPCurve)
                return initialConnector.Origin;
            
            // if it is a fitting...
            ConnectorManager connectorManager = (child as FamilyInstance).MEPModel.ConnectorManager;
            List<Connector> childConnectors = (connectorManager.Connectors).Cast<Connector>().ToList();

            if (childConnectors.Count == 2)
            {
                // find connector that is not initialConnector
                Connector queryResultConnector = childConnectors.First(x =>
                    x.Origin.DistanceTo(initialConnector.Origin) > settings.DistanceTolerance);
                
                // if it's connected to something do recursive call
                if (queryResultConnector.IsConnected)
                {
                    //check if child is connected to an mep curve or a family instance
                    List<Connector> subChildConnectors = (queryResultConnector.AllRefs).Cast<Connector>().ToList();
                    Connector subChildConnector = subChildConnectors.First(x => x.Owner.Id != queryResultConnector.Owner.Id);
                    if (subChildConnector.Owner is MEPCurve)
                        return ((child.Location) as LocationPoint).Point;
                    else
                        return EndChainPoint(queryResultConnector);
                }
                    
                // else return it's location
                else
                    return queryResultConnector.Origin;
            }
            else // if fitting is a T or a cross or anything with more than 2 connectors
            {
                return ((child.Location) as LocationPoint).Point;
            }
        }
