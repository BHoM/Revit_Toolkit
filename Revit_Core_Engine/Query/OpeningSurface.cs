...
namespace BH.Revit.Engine.Core
{
    public static partial class Query
    {
        ...
        public static ISurface OpeningSurface(this FamilyInstance familyInstance, HostObject host = null, RevitSettings settings = null)
        {
            ...
            long parentId = -1;
            if (parentElem != null)
                parentId = familyInstance.SuperComponent.Id.Value();
            ...
            else
            {
                ...
                hosts = hosts.Where(x => x.FindInserts(true, true, true, true).Any(y => (y.Value() == familyInstance.Id.Value()) || (y.Value() == parentId))).ToList();
            }
        ...
        private static List<ISurface> OpeningSurfaces_HostDocument(this FamilyInstance familyInstance, List<HostObject> hosts, Element parentElement, RevitSettings settings = null)
        {
            ...
            if (parentElement != null) // Indicates element is a nested instance and needs different approach to extract location
            {
                ...
                if (hosts.Count != 0)
                {
                    ...
                    List<ElementId> inserts = hosts.SelectMany(x => x.FindInserts(true, true, true, true)).Distinct().Where(x => x.Value() != -1).ToList();
            ...
            else if (hosts.Count != 0)
            {
                ...
                List<ElementId> inserts = hosts.SelectMany(x => x.FindInserts(true, true, true, true)).Distinct().Where(x => x.Value() != familyInstance.Id.Value()).ToList();
                doc.Delete(inserts);
                doc.Regenerate();
        ...
        private static List<ISurface> OpeningSurfaces_LinkDocument(this FamilyInstance familyInstance, List<HostObject> hosts, Element parentElement, RevitSettings settings = null)
        {
            ...
            else if (hosts.Count != 0)
            {
                ...
                foreach (HostObject host in hosts)
                {
                    ...
                    List<Autodesk.Revit.DB.Face> hostFaces = host.Faces(new Options(), settings);

                    // Find faces that are generated by the current family instance (opening edges)
                    List<Autodesk.Revit.DB.Face> edgeFaces = hostFaces.Where(x => host.GetGeneratingElementIds(x).Any(y => y.Value() == familyInstance.Id.Value())).ToList();

                    // If only one edge face is found and it has a downward-facing normal (Z = -1), look for additional connected faces
                    if (edgeFaces.Count == 1 && edgeFaces[0] is PlanarFace edgeFace && edgeFace.FaceNormal.IsAlmostEqualTo(new XYZ(0, 0, -1)))
                    {
                        BoundingBoxUV edgeFaceBbox = edgeFace.GetBoundingBox();
                        XYZ center = (edgeFace.Evaluate(edgeFaceBbox.Min) + edgeFace.Evaluate(edgeFaceBbox.Max)) / 2;
                        XYZ hostDirection = ((host?.Location as LocationCurve)?.Curve as Autodesk.Revit.DB.Line)?.Direction;
        ...
        {
            BH.Engine.Base.Compute.RecordWarning(String.Format("Geometry extraction of unhosted Windows is not currently supported. Revit ElementId: {0}", familyInstance.Id.Value()));
            List<ISurface> surfaces = new List<ISurface>();
            return surfaces;
        }
        ...
        private static List<ISurface> GetOpeningGeometry(Transaction t, Document doc, List<HostObject> hosts, List<ElementId> inserts, FamilyInstance familyInstance, RevitSettings settings = null)
        {
            ...
            if (loops != null)
                surfaces.AddRange(loops.Select(x => new PlanarSurface(x.FromRevit(), new List<ICurve>())));
