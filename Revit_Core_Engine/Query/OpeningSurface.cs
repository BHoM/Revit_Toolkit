        ...
        private static List<ISurface> OpeningSurfaces_LinkDocument(this FamilyInstance familyInstance, List<HostObject> hosts, Element parentElement, RevitSettings settings = null)
        {
            ...
            else if (hosts.Count != 0)
            {
                ...
                {                    
                    ...
                    if (edgeFaces.Count == 1  && (edgeFaces.FirstOrDefault() as PlanarFace).FaceNormal.IsAlmostEqualTo(new XYZ (0,0,-1)))
                    {
                        Autodesk.Revit.DB.Face edgeFace = edgeFaces.FirstOrDefault();                        
                        hostFaces.Remove(edgeFace);                        
                        BoundingBoxUV bbox = edgeFace.GetBoundingBox();
                        XYZ center = (edgeFace.Evaluate(bbox.Min) + edgeFace.Evaluate(bbox.Max)) / 2;                        
